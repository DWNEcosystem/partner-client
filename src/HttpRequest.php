...
//        }
    ...
    private function setCurlUrlMethod($handle, $url, $params, int $isPost = 0)
    {
        if ($isPost === 1) {
            ...
            \curl_setopt($handle, CURLOPT_POSTFIELDS, json_encode(["params"=>$params]));
     ...
     * @throws Exception
     */
    protected function post($url, $params, array $header = []):array
    {
        array_push($header,'sign:'.$this->MakeSign($params));
        $handle = \curl_init();
